...
class QueueDisplay extends UIComponent{
  ...
  private queue1DisplayValues: string[] = new Array(5).fill("- - - -");
  private queue2DisplayValues: string[] = new Array(5).fill("- - - -");
  ...
  initializeUI(): UINode {
    ...
    return View({
      children: [
          View({children: [
              Text({text: "Players in next game:", style: {...textProperties}}),
  ...
  private updateQueueDisplays() {
    for (let i = 0; i <= 4; i++) {
      this.queue1DisplayValues[i] = this.queue1[i]?.name.get() ?? "- - - -";
      this.queue2DisplayValues[i] = this.queue2[i]?.name.get() ?? "- - - -";
    }
    this.queue1DisplayBinding.set(this.queue1DisplayValues);
    this.queue2DisplayBinding.set(this.queue2DisplayValues);
